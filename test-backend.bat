@echo off
echo ===================================================
echo   TonTrac / WeighTruck - Running Backend Tests
echo ===================================================
echo.
...
cd "%~dp0backend"
echo [INFO] Running tests using %MVN_CMD%...
call %MVN_CMD% test
if %ERRORLEVEL% neq 0 (
    echo.
    echo [ERROR] Maven tests failed. Please review the output above.
) else (
    echo.
    echo [SUCCESS] All Spring Boot backend tests passed successfully!
)
pause

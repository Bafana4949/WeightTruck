document.addEventListener('DOMContentLoaded', async () => {
    ...
    const switchView = (viewName) => {
        ...
        if (viewName === 'fleets')
            renderFleetsTab();
        if (viewName === 'weighbridge-sim')
            initWeighbridgeSim();
        if (viewName === 'reconciliation')
            renderReconciliationTab();
        if (viewName === 'alerts-center')
            renderAlertsCenterTab();
        if (viewName === 'billing')
            renderBillingTab();
        if (viewName === 'driver-portal')
            initDriverPortal();
    };
    navItems.forEach(item => {
    ...
    const updateMassDisplays = () => {
        ...
        if (displayAllocated)
            displayAllocated.textContent = allocated.toFixed(2) + ' t';
        if (displayUnallocated)
            displayUnallocated.textContent = Math.max(0, appState.totalOrderMass - allocated).toFixed(2) + ' t';
        if (displayTotal)
            displayTotal.textContent = appState.totalOrderMass.toFixed(2) + ' t';
        if (maxAvail)
            maxAvail.textContent = Math.max(0, appState.totalOrderMass - allocated).toFixed(2);
    };
    // Modal Control Setup
    ...
    const renderFeed = async (overrideItems = null) => {
        ...
        items.forEach((t) => {
            ...
            if (t.status === 'Complete' || t.status === 'Accepted')
                badgeClass = 'complete';
            if (t.status === 'In Progress')
                badgeClass = 'in-progress';
            if (t.status === 'Variance Flagged' || t.status === 'Review Req')
    ...
    const animateTelemetry = () => {
        if (!movingTruck || !svgRouteLine || !appState.simulationActive)
            return;
    ...
    const triggerSimulationEvent = async () => {
        ...
        if (useBackend) {
            ...
            if (mockGateEvents.length > 5)
                mockGateEvents.pop();
        }
        renderFeed();
        renderGateEvents();
        const activeView = document.querySelector('.view-section.active');
        if (activeView && activeView.id === 'inventory-view')
            renderInventoryTab();
    };
    ...
    const renderOrdersTable = async () => {
        ...
        const filtered = orders.filter(o => (o.orderId || o.id || '').toLowerCase().includes(filterVal) ||
    ...
    const renderInventoryTab = async () => {
        if (!stockpilesGridContainer || !inventoryLogsBody)
            return;
    ...
    const renderReportsCharts = () => {
        ...
        if (barChartContent) {
            ...
            prodKeys.forEach((key, index) => {
                ...
                if (key === 'Anthracite Pea')
                    rect.style.fill = 'var(--primary-blue)';
                if (key === 'RB1 Export')
                    rect.style.fill = 'var(--primary-amber)';
                if (key === 'Duff')
                    rect.style.fill = 'var(--primary-cyan)';
    ...
    const renderSettingsTab = () => {
        if (toggleSimSwitch)
            toggleSimSwitch.checked = appState.simulationActive;
        if (simSpeedRange)
            simSpeedRange.value = appState.simulationInterval.toString();
        if (varThresholdInput)
            varThresholdInput.value = appState.varianceThreshold.toString();
        if (ratePerTonInput)
            ratePerTonInput.value = appState.ratePerTon.toString();
        if (dailyTargetInput)
            dailyTargetInput.value = appState.dailyTarget.toString();
    };
    if (saveSettingsBtn) {
    ...
    const updateAnalytics = (id) => {
        ...
        if (dNo)
            dNo.textContent = data.dispatch.no;
        if (dSrc)
            dSrc.textContent = data.dispatch.src || '';
        if (dWt)
            dWt.textContent = data.dispatch.weight.toFixed(2) + ' t';
        if (dTm)
            dTm.textContent = data.dispatch.time;
        if (rNo)
            rNo.textContent = data.receipt.no;
        if (rDest)
            rDest.textContent = data.receipt.dest || '';
        if (rWt)
            rWt.textContent = data.receipt.weight.toFixed(2) + ' t';
        if (rTm)
            rTm.textContent = data.receipt.time;
        if (vVal)
            vVal.textContent = data.variance.toFixed(1) + '%';
        if (statusEl) {
    ...
    const filterFeed = () => {
        ...
        const filtered = mockFeed.filter((t) => t.id.toLowerCase().includes(query) ||
    ...
    if (feedSearchInput)
        feedSearchInput.oninput = filterFeed;
    if (mainSearchInput)
        mainSearchInput.oninput = filterFeed;
    if (btnViewAllFeed) {
    ...
    const updateOrderMass = () => {
        ...
        if (totalMassEl)
            totalMassEl.textContent = mass.toFixed(2) + ' t';
        updateMassDisplays();
    };
    if (comboProduct) {
        comboProduct.onchange = (e) => {
            ...
            if (product && product.id == 10) {
                if (orderNoteContainer)
                    orderNoteContainer.style.display = 'block';
                if (orderNoteText)
                    orderNoteText.textContent = "Moisture adjustment factor (0.5%) will be applied to this product.";
            }
            else {
                if (orderNoteContainer)
                    orderNoteContainer.style.display = 'none';
            }
        };
    }
    if (orderEstimatedMass) {
    ...
    document.querySelectorAll('input[name="IsReceipt"]').forEach(radio => {
        ...
        htmlRadio.onchange = (e) => {
            ...
            if (customerCombo)
                customerCombo.disabled = isReceipt;
            if (supplierCombo)
                supplierCombo.disabled = !isReceipt;
            if (isReceipt) {
                if (customerCombo)
                    customerCombo.value = userInfo.organisationId.toString();
            }
            else {
                if (supplierCombo)
                    supplierCombo.value = userInfo.organisationId.toString();
            }
        };
    });
    ...
    const openVehicleModal = (index) => {
        ...
        if (vehicleModalTitle)
            vehicleModalTitle.textContent = `Authorised Vehicles: ${appState.hauliers[index].name}`;
    ...
    const renderVehiclePickers = () => {
        ...
        appState.availableVehicles.forEach(v => {
            if (showOnlyInducted && !v.inducted)
                return;
            const item = document.createElement('div');
        ...
        if (displayAvailable)
            displayAvailable.textContent = availList.children.length.toString();
        if (displayAssigned)
            displayAssigned.textContent = assignedList.children.length.toString();
        renderHaulierGrid();
    };
    const toggleVehicle = (id) => {
        if (!appState.assignedVehicles[activeHaulierIndex])
            ...
            if (activeSection && activeSection.id === 'orders-view') {
                ...
                if (activeOrdersEl) {
                    if (useBackend) {
                        const res = await fetch(`${BACKEND_URL}/orders`);
                        if (res.ok)
                            activeOrdersEl.textContent = (await res.json()).length.toString();
                    }
                    else {
    ...
    let btnExitDriverView = document.getElementById('btn-exit-driver-view');
    ...
    const applyRolePermissions = (role) => {
        ...
        if (role === 'Driver') {
            ...
            if (btnExitDriverView)
                btnExitDriverView.style.display = 'block';
        }
        else {
            if (sidebar)
        ...
        navItemsList.forEach(item => {
            const htmlItem = item;
            const view = htmlItem.dataset.view;
            if (htmlItem.classList.contains('logout'))
                return;
    ...
    const toggleWriteControls = (enable) => {
        const writeButtons = [
            'btn-create-order',
    ...
    const renderSitesTab = async () => {
        ...
        const filterVal = (searchInput?.value || '').trim().toLowerCase();
        const filtered = sites.filter(s => (s.name || '').toLowerCase().includes(filterVal) ||
    ...
    const openSiteCrudModal = (site) => {
        const modal = document.getElementById('site-crud-modal');
        if (!modal)
            return;
        const title = document.getElementById('site-modal-title');
        const form = document.getElementById('site-crud-form');
        if (form)
            form.reset();
        if (site) {
            if (title)
                title.textContent = "Edit Site Details";
            document.getElementById('site-crud-id').value = (site.id || '').toString();
            document.getElementById('site-crud-name').value = site.name;
            document.getElementById('site-crud-type').value = site.type;
            document.getElementById('site-crud-lat').value = site.latitude.toString();
            document.getElementById('site-crud-lng').value = site.longitude.toString();
            document.getElementById('site-crud-hours').value = site.operatingHours;
            document.getElementById('site-crud-rules').value = site.accessRules;
        }
        else {
            if (title)
                title.textContent = "Register New Site";
            document.getElementById('site-crud-id').value = "";
        }
        modal.style.display = 'block';
    };
    ...
    if (btnSaveSiteCrud) {
        btnSaveSiteCrud.onclick = async (e) => {
            e.preventDefault();
            const idVal = document.getElementById('site-crud-id').value;
            const nameVal = document.getElementById('site-crud-name').value;
            const typeVal = document.getElementById('site-crud-type').value;
            const latVal = parseFloat(document.getElementById('site-crud-lat').value || '0');
            const lngVal = parseFloat(document.getElementById('site-crud-lng').value || '0');
            const hoursVal = document.getElementById('site-crud-hours').value;
            const rulesVal = document.getElementById('site-crud-rules').value;
            if (!nameVal) {
    ...
    const renderFleetActiveTab = () => {
        if (activeFleetTab === 'hauliers')
            renderHauliersFleetTab();
        if (activeFleetTab === 'vehicles')
            renderVehiclesFleetTab();
        if (activeFleetTab === 'drivers')
            renderDriversFleetTab();
    };
    ...
    const renderHauliersFleetTab = async () => {
        ...
        const searchInput = document.getElementById('hauliers-search');
        const filterVal = (searchInput?.value || '').trim().toLowerCase();
        const filtered = hauliers.filter(h => (h.name || '').toLowerCase().includes(filterVal) ||
    ...
    const openHaulierCrudModal = (h) => {
        const modal = document.getElementById('haulier-crud-modal');
        if (!modal)
            return;
        const title = document.getElementById('haulier-crud-modal-title');
        const form = document.getElementById('haulier-crud-form');
        if (form)
            form.reset();
        if (h) {
            if (title)
                title.textContent = "Edit Haulier Details";
            document.getElementById('haulier-crud-id').value = h.id;
            document.getElementById('haulier-crud-name').value = h.name;
            document.getElementById('haulier-crud-email').value = h.email || '';
            document.getElementById('haulier-crud-phone').value = h.phone || '';
            document.getElementById('haulier-crud-compliance').value = h.complianceStatus || 'Compliant';
        }
        else {
            if (title)
                title.textContent = "Register New Haulier";
            document.getElementById('haulier-crud-id').value = "";
        }
        modal.style.display = 'block';
    };
    ...
    if (btnSaveHaulierCrud) {
        btnSaveHaulierCrud.onclick = async (e) => {
            e.preventDefault();
            const idVal = document.getElementById('haulier-crud-id').value;
            const nameVal = document.getElementById('haulier-crud-name').value;
            const emailVal = document.getElementById('haulier-crud-email').value;
            const phoneVal = document.getElementById('haulier-crud-phone').value;
            const complianceVal = document.getElementById('haulier-crud-compliance').value;
            if (!nameVal) {
    ...
    const hauliersSearch = document.getElementById('hauliers-search');
    if (hauliersSearch) {
    ...
    const renderVehiclesFleetTab = async () => {
        ...
        const filterVal = (searchInput?.value || '').trim().toLowerCase();
        const filtered = vehicles.filter(v => (v.reg || '').toLowerCase().includes(filterVal) ||
    ...
    const openVehicleCrudModal = (v) => {
        ...
        if (v) {
            ...
            document.getElementById('vehicle-crud-id').value = v.id.toString();
            document.getElementById('vehicle-crud-reg').value = v.reg;
            document.getElementById('vehicle-crud-fleet').value = v.fleet || '';
            document.getElementById('vehicle-crud-haulier').value = appState.masterHauliers.find(h => h.name === v.haulier)?.id || "";
            document.getElementById('vehicle-crud-type').value = v.type || 'TRUCK';
            document.getElementById('vehicle-crud-weight').value = (v.maxLegalWeight || 50.0).toString();
            document.getElementById('vehicle-crud-compliance').value = v.complianceStatus || 'Compliant';
            document.getElementById('vehicle-crud-gps').value = v.gpsDeviceId || '';
        }
        else {
            if (title)
                title.textContent = "Register New Vehicle";
    ...
    if (btnSaveVehicleCrud) {
        btnSaveVehicleCrud.onclick = async (e) => {
            e.preventDefault();
            const idVal = document.getElementById('vehicle-crud-id').value;
            const regVal = document.getElementById('vehicle-crud-reg').value;
            const fleetVal = document.getElementById('vehicle-crud-fleet').value;
            const hSelect = document.getElementById('vehicle-crud-haulier');
            const haulierVal = hSelect.selectedOptions[0]?.text || '';
            const typeVal = document.getElementById('vehicle-crud-type').value;
            const weightVal = parseFloat(document.getElementById('vehicle-crud-weight').value || '50.0');
            const complianceVal = document.getElementById('vehicle-crud-compliance').value;
            const gpsVal = document.getElementById('vehicle-crud-gps').value;
            if (!regVal) {
    ...
    const renderDriversFleetTab = async () => {
        ...
        const filterVal = (searchInput?.value || '').trim().toLowerCase();
        const filtered = drivers.filter(d => (d.name || '').toLowerCase().includes(filterVal) ||
    ...
    const openDriverCrudModal = (d) => {
        const modal = document.getElementById('driver-crud-modal');
        if (!modal)
            return;
        const title = document.getElementById('driver-crud-modal-title');
        const form = document.getElementById('driver-crud-form');
        if (form)
            form.reset();
        populateHaulierDropdowns();
        if (d) {
            if (title)
                title.textContent = "Edit Driver Details";
            document.getElementById('driver-crud-id').value = (d.id || '').toString();
            document.getElementById('driver-crud-name').value = d.name;
            document.getElementById('driver-crud-idnum').value = d.idNumber;
            document.getElementById('driver-crud-licensenum').value = d.licenseNumber;
            document.getElementById('driver-crud-haulier').value = appState.masterHauliers.find(h => h.name === d.assignedHaulier)?.id || "";
            document.getElementById('driver-crud-induction').value = d.inductionStatus;
            document.getElementById('driver-crud-medical').value = d.medicalExpiryDate || '';
            document.getElementById('driver-crud-training').value = d.trainingExpiryDate || '';
        }
        else {
            if (title)
                title.textContent = "Register New Driver";
            document.getElementById('driver-crud-id').value = "";
        }
        modal.style.display = 'block';
    };
    ...
    if (btnSaveDriverCrud) {
        btnSaveDriverCrud.onclick = async (e) => {
            e.preventDefault();
            const idVal = document.getElementById('driver-crud-id').value;
            const nameVal = document.getElementById('driver-crud-name').value;
            const idnumVal = document.getElementById('driver-crud-idnum').value;
            const licensenumVal = document.getElementById('driver-crud-licensenum').value;
            const hSelect = document.getElementById('driver-crud-haulier');
            const haulierVal = hSelect.selectedOptions[0]?.text || '';
            const inductionVal = document.getElementById('driver-crud-induction').value;
            const medicalVal = document.getElementById('driver-crud-medical').value;
            const trainingVal = document.getElementById('driver-crud-training').value;
    ...
    const populateHaulierDropdowns = () => {
        const orderHaulierSelect = document.getElementById('comboHaulierId');
        const simHaulierSelect = document.getElementById('sim-haulier-select');
        const vehicleCrudHaulier = document.getElementById('vehicle-crud-haulier');
        const driverCrudHaulier = document.getElementById('driver-crud-haulier');
    ...
    const initWeighbridgeSim = () => {
        simStep = 1;
        updateSimTimelineUI();
        const orderSelect = document.getElementById('sim-order-select');
        if (orderSelect) {
            ...
            orderSelect.onchange = () => {
                ...
                const haulierSelect = document.getElementById('sim-haulier-select');
                const vehicleSelect = document.getElementById('sim-vehicle-select');
                const driverInput = document.getElementById('sim-driver-input');
                if (order) {
                    if (haulierSelect) {
    ...
    const updateSimTimelineUI = () => {
        for (let i = 1; i <= 5; i++) {
            ...
            if (pane)
                pane.style.display = i === simStep ? 'block' : 'none';
    ...
    const runComplianceClearanceCheck = () => {
        ...
        const nextBtn = document.getElementById('btn-sim-next-2');
        if (!banner || !details)
            return;
    ...
    const updateSimPrintPreview = () => {
        ...
        const printBtn = document.getElementById('btn-sim-print-ticket');
        if (pNo)
            pNo.textContent = simStep === 5 ? 'T-' + Math.floor(80000 + Math.random() * 10000) : 'T-XXXXX';
        if (pDate)
            pDate.textContent = new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' });
        if (pReg)
            pReg.textContent = simTicket.reg || '------';
        if (pTrailer)
            pTrailer.textContent = simTicket.trailerReg || '------';
        if (pDriver)
            pDriver.textContent = simTicket.driver || '------';
        if (pHaulier)
            pHaulier.textContent = simTicket.haulier || '------';
        if (pOrigin)
            pOrigin.textContent = simTicket.origin || '------';
        if (pProduct)
            pProduct.textContent = simTicket.product || '------';
        if (pGross)
            pGross.textContent = simTicket.grossWeight ? `${simTicket.grossWeight.toFixed(2)} t` : '0.00 t';
        if (pTare)
            pTare.textContent = simTicket.tareWeight ? `${simTicket.tareWeight.toFixed(2)} t` : '0.00 t';
        if (pNet)
            pNet.textContent = simTicket.netWeight ? `${simTicket.netWeight.toFixed(2)} t` : '0.00 t';
        if (pStatus) {
            if (simStep < 5) {
    ...
    if (btnSimNext1) {
        btnSimNext1.onclick = (e) => {
            e.preventDefault();
            const orderVal = document.getElementById('sim-order-select').value;
            const haulierVal = document.getElementById('sim-haulier-select').value;
            const vehicleVal = document.getElementById('sim-vehicle-select').value;
            const driverVal = document.getElementById('sim-driver-input').value;
            const trailerVal = document.getElementById('sim-trailer-input').value;
    ...
    if (btnSimCaptureTare) {
        btnSimCaptureTare.onclick = (e) => {
            e.preventDefault();
            const tareVal = parseFloat(document.getElementById('sim-tare-weight-input').value || '14.5');
            const display = document.getElementById('sim-tare-display');
            if (display)
                display.innerHTML = `${tareVal.toFixed(2)} <small>t</small>`;
            simTicket.tareWeight = tareVal;
            const nextBtn = document.getElementById('btn-sim-next-3');
            if (nextBtn)
                nextBtn.disabled = false;
    ...
    if (btnSimCaptureGross) {
        btnSimCaptureGross.onclick = (e) => {
            e.preventDefault();
            const grossVal = parseFloat(document.getElementById('sim-gross-weight-input').value || '48.5');
            const display = document.getElementById('sim-gross-display');
            if (display)
                display.innerHTML = `${grossVal.toFixed(2)} <small>t</small>`;
            simTicket.grossWeight = grossVal;
            simTicket.netWeight = grossVal - simTicket.tareWeight;
            const destVal = parseFloat(document.getElementById('sim-dest-weight-input').value || '33.8');
            ...
            const nextBtn = document.getElementById('btn-sim-next-4');
            if (nextBtn)
                nextBtn.disabled = false;
    ...
    if (btnSimReset) {
        btnSimReset.onclick = (e) => {
            ...
            if (tareDisp)
                tareDisp.innerHTML = `0.00 <small>t</small>`;
            const grossDisp = document.getElementById('sim-gross-display');
            if (grossDisp)
                grossDisp.innerHTML = `0.00 <small>t</small>`;
            const nextBtn3 = document.getElementById('btn-sim-next-3');
            if (nextBtn3)
                nextBtn3.disabled = true;
            const nextBtn4 = document.getElementById('btn-sim-next-4');
            if (nextBtn4)
    ...
    const renderReconciliationTab = () => {
        ...
        list.forEach(t => {
            ...
            if (v < 1.0)
                matchedCount++;
            else if (v <= appState.varianceThreshold)
                minorCount++;
            else
                majorCount++;
        });
        ...
        if (matchedKPI)
            matchedKPI.textContent = matchedCount.toString();
        if (minorKPI)
            minorKPI.textContent = minorCount.toString();
        if (majorKPI)
            majorKPI.textContent = majorCount.toString();
        const searchVal = (document.getElementById('recon-search')?.value || '').trim().toLowerCase();
        const filterVal = (document.getElementById('recon-status-filter')?.value || 'ALL');
        const filtered = list.filter(t => {
            ...
            if (filterVal === 'Matched')
                matchesFilter = t.variance < 1.0;
            else if (filterVal === 'Minor Variance')
                matchesFilter = t.variance >= 1.0 && t.variance <= appState.varianceThreshold;
            else if (filterVal === 'Major Discrepancy')
                matchesFilter = t.variance > appState.varianceThreshold;
            else if (filterVal === 'Unreconciled')
                matchesFilter = t.status === 'Review Req';
    ...
    const renderAlertsCenterTab = () => {
        ...
        const severityFilter = document.getElementById('alert-severity-filter')?.value || 'ALL';
        const filtered = list.filter(w => {
            if (severityFilter === 'ALL')
                return true;
            if (severityFilter === 'CRITICAL')
                return w.title.toLowerCase().includes('critical') || w.type === 'alert' && w.desc.toLowerCase().includes('access');
            if (severityFilter === 'HIGH')
                return w.type === 'alert' && !w.title.toLowerCase().includes('critical');
            if (severityFilter === 'MEDIUM')
                return w.type === 'info';
            if (severityFilter === 'LOW')
                return w.type === 'success';
            return true;
        });
        if (filtered.length === 0) {
    ...
    const renderBillingTab = () => {
        ...
        if (tonsKPI)
            tonsKPI.innerHTML = `${totalTons.toFixed(2)} <small>t</small>`;
        if (revenueKPI)
            revenueKPI.textContent = `R ${totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
        if (countKPI)
            countKPI.textContent = billingReadyCount.toString();
        const filterVal = document.getElementById('billing-status-filter')?.value || 'ALL';
        const filtered = list.filter(t => {
    ...
    let driverProgressInterval = null;
    ...
    const initDriverPortal = () => {
        ...
        if (nameDisplay)
            nameDisplay.textContent = 'Musa Ndiaye';
        if (regDisplay)
            regDisplay.textContent = 'GP-142-TR';
        if (qrReg)
            qrReg.textContent = 'GP-142-TR';
        ...
        if (appState.orders.length > 0) {
            ...
            if (tripBox) {
                ...
                if (drvTripId)
                    drvTripId.textContent = currentDriverTrip.orderId || currentDriverTrip.id || '';
                if (drvTripType)
                    drvTripType.textContent = currentDriverTrip.type.toUpperCase();
                if (drvTripProduct)
                    drvTripProduct.textContent = currentDriverTrip.product;
                if (drvTripClient)
                    drvTripClient.textContent = currentDriverTrip.client;
                if (drvTripMass)
                    drvTripMass.textContent = `${currentDriverTrip.target.toFixed(2)} t Target`;
            }
        }
        else {
            if (noTrip)
    ...
    const updateDriverScreenUI = () => {
        ...
        screens.forEach((sc, idx) => {
            ...
            if (el)
                el.classList.toggle('active', (idx + 1) === driverStep);
        });
    };
            ...
            driverProgressInterval = setInterval(() => {
                if (driverProgress < 100) {
                    ...
                    if (driverProgress >= 100) {
                        clearInterval(driverProgressInterval);
                        if (statusText)
                            statusText.textContent = 'Arrived at Scale Gate';
                        if (checkinBtn)
                        ...
                        if (statusText)
                            statusText.textContent = `En Route (${driverProgress}%)`;
                    }
                }
            }, 800);
        };
        ...
        btnDriverGateCheckin.onclick = (e) => {
            ...
            setTimeout(() => {
                ...
                if (isCompliant) {
                    ...
                    const weighBtn = document.getElementById('btn-driver-simulate-weigh');
                    if (weighBtn)
    ...
    if (btnDriverCompleteTrip) {
        btnDriverCompleteTrip.onclick = async (e) => {
            e.preventDefault();
            const dropoffSP = document.getElementById('drv-dropoff-sp-select').value;
            const receiptWeight = parseFloat(document.getElementById('drv-receipt-weight-input').value || '33.8');
            const ticketId = 'T-' + Math.floor(80000 + Math.random() * 10000);
            const dispNo = 'D-' + Math.floor(10000 + Math.random() * 90000);
            const rectNo = 'R-' + Math.floor(10000 + Math.random() * 90000);
            const timeStr = new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' });
